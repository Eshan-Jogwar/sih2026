...
pub struct Text<O: ObjectStore> {
    id: Uuid,
    key: String,
    storage: O,
}

impl<O: ObjectStore> Text<O> {
...
#[async_trait]
impl<O: ObjectStore> Document for Text<O> {
    fn id(&self) -> Uuid {
        self.id
    }
    ...
    async fn update_extracted_information(
        ...
        if result.is_none() {
            return Err(DocumentErrors::NotFound(String::from("did not find the text document")));
        }
